...
export function retrievePrivateQuizzes(username) {
  return async (dispatch,getState) => {
    ...
    jsonResponse.forEach(quizData => {
      if (getState().privateQuizzes.some(pQuiz => pQuiz.id === quizData.id)) console.log("private quiz clash: "+quizData.id);
...
export function deletePrivateQuiz(quiz) {
  return async (dispatch, getState) => {
    const response = await fetcher("/quiz/"+quiz.data.id,
      {method: "DELETE", credentials:"include", headers: {"Content-Type":"application/json", "Connection":"keep-alive"}, body:JSON.stringify(quiz)});
    const jsonResponse = await response.json();
    if (jsonResponse.message === "quiz delete") dispatch({type:"privateQuizzes/deleteQuiz",payload:quiz.data.id});
  }
}
